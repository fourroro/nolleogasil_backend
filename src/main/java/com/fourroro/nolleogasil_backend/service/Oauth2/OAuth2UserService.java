...
public class OAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String subject = (String) oAuth2User.getAttributes().get(userNameAttributeName);
    ...
    private Users saveOrUpdate(OAuth2Attribute oAuth2Attribute) {
        ...
        // 전화번호나 다른 추가 정보로 동일 사용자인지 확인
        Optional<Users> existingUserByPhone = usersRepository.findByPhone(oAuth2Attribute.getPhone());
        if (existingUserByPhone.isPresent()) {
            // 동일한 전화번호를 가진 사용자가 이미 존재할 경우 중복 가입 제한
            return existingUserByPhone.get();
        }
        ...
        if (users.getRole() == null) {
            users.setRole(Role.USER); // 기본 역할 설정
        }
